...
export function buildVerificationSystemPrompt(): string {
  return [
    ...
    'code that it is benign (quote the exact line or construct). For a finding that arrives',
    "WITHOUT file content, the only evidence available is the finding's own stated facts (its",
    'description, counts, paths, metadata) — confirm a false positive there only if those facts',
    'themselves show it is benign, and never assume code you were not shown. If you cannot cite',
    'such evidence — or the evidence is ambiguous — do NOT confirm the false positive:',
